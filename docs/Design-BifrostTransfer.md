...
* **概述**：
    ...
    * **技术选型**：Flutter 框架（版本 3.0.6+）、Provider 状态管理、Socket 通信、SQLite 数据存储。
...
2. 点击"连接设备"按钮
   - **按钮点击后立即变为禁用状态，文本变为"连接中..."，显示加载动画**
   - 状态栏更新为"连接中..."，颜色变为黄色
3. 系统弹出配对码对话框，显示6位数字配对码
   - 对话框包含配对码显示、倒计时（60秒）和"取消"按钮
4. 连接成功后：
   - **配对码对话框自动关闭**
   - 状态栏更新为"已连接"，颜色变为绿色
   - 显示已连接设备信息
   - **"连接设备"按钮文本变为"断开连接"，恢复可用状态**
5. 连接失败情况：
   - 超时：倒计时结束后，**对话框自动关闭**，显示"连接超时"提示
   - 被拒绝：**对话框自动关闭**，显示"连接被拒绝"提示
   - IP错误：**对话框自动关闭**，显示"无法连接到指定IP"提示
   - 以上所有情况，按钮恢复为"连接设备"，状态栏显示"连接失败"，颜色变为红色
...
- **连接超时**：
  - 连接请求发出后60秒内未收到响应，判定为超时
  - **配对码对话框自动关闭**，显示"连接超时"提示
  - 状态栏更新为"连接失败"

- **连接被拒绝**：
  - 接收方拒绝连接请求
  - **配对码对话框自动关闭**，显示"连接被拒绝"提示
  - 状态栏更新为"连接失败"
