...
class ConnectionServiceImpl implements ConnectionService {
  ...
  ConnectionServiceImpl(this._socketService) {
    // 订阅Socket消息
    _messageSubscription = _socketService.messageStream.listen(_handleSocketMessage);
    
    // 订阅Socket连接状态
    _connectionStatusSubscription = _socketService.connectionStatusStream.listen(_handleSocketConnectionStatus);
  ...
  Future<String> initiateConnection(String targetIp) async {
    try {
      ...
      _connectionTimer = Timer(
        Duration(milliseconds: NetworkConstants.CONNECTION_REQUEST_TIMEOUT_MS), 
        () {
      ...
      // 设置配对确认超时
      _pairingTimer = Timer(
        Duration(milliseconds: NetworkConstants.PAIRING_CONFIRMATION_TIMEOUT_MS), 
        () {
  ...
  Future<bool> acceptConnection(String initiatorIp, String pairingCode) async {
    try {
      ...
      // 发送接受连接响应
      final connectionResponseMessage = SocketMessageModel.createConnectionResponse(
        accepted: true,
        deviceName: localDeviceInfo.deviceName,
        deviceIp: localDeviceInfo.ipAddress,
      );
  ...
  Future<bool> rejectConnection(String initiatorIp, [String? reason]) async {
    try {
      ...
      _logger.info('拒绝来自 $initiatorIp 的连接请求${reason != null ? '，原因: $reason' : ''}');
  ...
  void _handleSocketConnectionStatus(bool connected) {
    ...
    if (!connected && _currentConnectionState.status == ConnectionStatus.connected) {
