...
class SocketCommunicationServiceImpl implements SocketCommunicationService {
  ...
  Future<void> startServer() async {
    ...
    try {
      _server = await ServerSocket.bind(
          InternetAddress.anyIPv4, NetworkConstants.LISTEN_PORT);
  ...
  Future<bool> _attemptConnection(String ip, int port) async {
    try {
      _logger.info(
          '尝试连接到设备: $ip:$port (尝试 ${_connectionRetryCount + 1}/${NetworkConstants.MAX_CONNECTION_RETRIES})');
      ...
      if (_connectionRetryCount < NetworkConstants.MAX_CONNECTION_RETRIES) {
        ...
        await Future.delayed(
            Duration(milliseconds: NetworkConstants.RETRY_INTERVAL_MS));
        return _attemptConnection(ip, port);
      }
  ...
  void _handleClientConnection(Socket client) {
    _logger
        .info('接收到客户端连接: ${client.remoteAddress.address}:${client.remotePort}');
  ...
  void _handleHeartbeatTimeout() {
    ...
    if (lastResponse == null ||
        now.difference(lastResponse).inMilliseconds >
            NetworkConstants.HEARTBEAT_TIMEOUT_MS) {
