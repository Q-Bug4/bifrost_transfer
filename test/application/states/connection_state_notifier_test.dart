...
void main() {
  ...
  setUp(() {
    ...
    // 模拟连接请求流
    final connectionRequestController =
  ...
  group('ConnectionStateNotifier - 初始化', () {
    test('初始化时应该获取本地设备信息', () async {
      ...
      expect(
          connectionStateNotifier.localDeviceInfo!.deviceName, 'Test Device');
      expect(
          connectionStateNotifier.localDeviceInfo!.ipAddress, '192.168.1.100');
    ...
    test('初始化时应该订阅连接状态流和连接请求流', () async {
      ...
      // 验证
      verify(mockConnectionService.connectionStateStream)
          .called(greaterThanOrEqualTo(1));
      verify(mockConnectionService.connectionRequestStream)
          .called(greaterThanOrEqualTo(1));
  ...
  group('ConnectionStateNotifier - 发起连接', () {
    ...
    test('发起连接失败时应该抛出异常', () async {
      ...
      // 执行测试并验证异常
      expect(() => connectionStateNotifier.initiateConnection(targetIp),
  ...
  group('ConnectionStateNotifier - 接受连接请求', () {
    ...
    test('接受连接请求成功', () async {
      ...
      // 验证
      verify(mockConnectionService.acceptConnection(initiatorIp, pairingCode))
          .called(1);
  ...
  group('ConnectionStateNotifier - 断开连接', () {
    test('断开连接成功', () async {
      // 准备
      when(mockConnectionService.disconnect()).thenAnswer((_) async => null);
